...
description: How the optimization worker consumes jobs and produces results — RabbitMQ contract, S3 flow, OSRM integration, and solver routing. Use when modifying job processing, queues, or solver wiring in solver-vgandolfi-dev-worker.
---
...
- Bucket: `settings.S3_BUCKET_NAME` (aligned with orchestrator: `solver-vgandolfi-dev`).
